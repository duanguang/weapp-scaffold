...
import { throttle, isH5 } from '@/utils/throttle'
/**
 * 长列表  引用自https://github.com/tingyuxuan2302/taro3-virtual-list
 */
export default class VirtialList extends Component<VirtualListProps, VirtualListState> {
  ...
  render(): JSX.Element {
    ...
    if (autoScrollTop) {
      scrollTopProps['scrollTop']=(innerScrollTop === 0 ? 0 : "")
    }
    else {
      /** fix: 修复在h5页面中如果未传scrollTop，导致加载下一页被重置回顶部 */
      if (Taro.getEnv() === Taro.ENV_TYPE.WEB) {
        if (scrollViewProps?.scrollTop!==void 0) {
           scrollTopProps['scrollTop'] = scrollViewProps?.scrollTop;
        }
      } else {
        scrollTopProps['scrollTop'] = scrollViewProps?.scrollTop;
      }
    ...
    return (
      ...
      >
        <View className="zt-scroll-content" style={{paddingBottom:`${Taro.pxTransform(80,750)}`}}>
          {onRenderTop?.()}
